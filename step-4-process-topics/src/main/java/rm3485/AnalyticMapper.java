...
public class AnalyticMapper extends Mapper<LongWritable, Text, Text, Text> {
  ...
  private double topicEqualsTag = 0.4;
  private double topicInBody = 0.3;
  private double topicInTitle = 0.3;
      ...
      if(split.length >= 5){
        ...
        if(score > 50){
          ...
          while(it.hasNext()){
            ...
            String date = data[0].trim().toLowerCase();
            String topic = data[1].trim().toLowerCase();
            String count = data[2].trim();
            double weight = 0.0;


            // FIXME: Look for whole words
            String spaced_topic = " " + topic + " ";
            if(body.contains(spaced_topic)){
              weight += topicInBody;
            }if(title.contains(spaced_topic)){
              weight += topicInTitle;
            }
