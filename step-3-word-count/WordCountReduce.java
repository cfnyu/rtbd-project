...
public class WordCountReduce
    ...
    public void reduce(Text key, Iterable<Text> values, Context context)
        ...
        String stop_expr = "^([0-9]|in|the|at|a|with|or|have|then|if|while|for|dotry|end|import|has|update|updated|updating|skipevent|field|icons|load|same|since|tag|pull|push|back|also|context|content|current|title|version|adding|add|added|delete|deleting|deleted|being|help|key|value|output|keys|values|outputs|than|branch|move|via|as|to|merge|from|and|fix|fixing|on|master|commit|emacs|code|use|not|fixed|by|see|be|time|changes|that|now|fixes|de|readme|it|-|of|new|is|when|some|more|support|up|change|all|latest|about|after|an|but|against|any|been|between|could|did|during|does|each|few|having|who|what|where|how|do|you|yourself|would|were|very|until|through|those|they|we|him|her|own|other|outselves|over|only|most|let|its|it's|me|myself|i|I|here's|had|doing|done|down|both|before|am|again|example|error|errors|empty|full|docs|doc|documentation|digital|develop|dev|description|dependency|dependencies|comment|comment|can|using|user|updates|unused|used|typo|typos|so|should|return|refactor|refactoring|one|handling)*$";
